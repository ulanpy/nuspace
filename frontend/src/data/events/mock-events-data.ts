...
export const todayEvents = [
  ...
  },
  {
    ...
    place: "Student Lounge",
    description:
      "Screening of an award-winning documentary followed by a discussion with a special guest speaker.",
    duration: 150,
...
export const academicEvents = mockEvents.filter(
  (event) =>
    event.club?.type === "academic" ||
    event.club?.type === "professional" ||
    event.club?.type === "technology",
);
export const culturalEvents = mockEvents.filter(
  (event) => event.club?.type === "cultural" || event.club?.type === "art",
);
export const sportsEvents = mockEvents.filter(
  (event) => event.club?.type === "sports",
);
export const socialEvents = mockEvents.filter(
  (event) =>
    event.club?.type === "social" || event.club?.type === "recreational",
);
export const featuredEvents = mockEvents.filter(
  (event) => (event.rating || 0) > 7.5,
);
  ...
  getEvents: (
    ...
    if (clubType) {
      filteredEvents = filteredEvents.filter(
        (event) => event.club?.type === clubType,
      );
    }
