...
import { sgotinishApi } from "@/features/sgotinish/api";
...
export function OtinishStats() {
  const { data, isLoading, isError } = useQuery({
    queryKey: ["sgotinish", "stats"],
    queryFn: sgotinishApi.getPublicStats,
    staleTime: 60_000,
  });
  ...
  return (
    <div className="space-y-8">
      <div>
        <p className="text-sm font-medium text-foreground">Already in use</p>
        <p className="mt-1 text-sm text-muted-foreground">
          Real appeals from NU students — always anonymous to Student Government.
        </p>
      </div>
      ...
      {topCategories.length > 0 && maxCat > 0 && (
        <div className="space-y-3">
          <p className="text-sm font-medium text-foreground">Where people write</p>
