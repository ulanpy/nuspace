...
from backend.routes.google_bucket.schemas import EntityType
...
async def add_new_product_to_db(
    ...
    return await build_product_response(new_product, session, request, entity_type)
...
async def get_products_of_user_from_db(
    ...
    return list(
        await asyncio.gather(
            *(
                build_product_response(product, session, request, entity_type)
...
async def show_products_from_db(
    ...
    products_response = await asyncio.gather(
        *(build_product_response(product, session, request, entity_type) for product in products)
    )
...
async def get_product_from_db(
    ...
    return await build_product_response(product, session, request, entity_type)
...
async def remove_product_from_db(
    ...
    if product:
        ...
        media_result = await session.execute(
            select(Media).filter(
                Media.entity_id == product.id, Media.entity_type == EntityType.products
            )
        )
