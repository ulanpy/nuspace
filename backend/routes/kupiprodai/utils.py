...
async def get_media_responses(
    ...
    media_result = await session.execute(
        select(Media).filter(Media.entity_id == product_id, Media.section == media_section)
    )
...
async def build_product_response(
    ...
    """
    media_responses = await get_media_responses(session, request, product.id, media_section)
