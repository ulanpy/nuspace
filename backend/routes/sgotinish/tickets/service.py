...
from backend.routes.sgotinish.tickets import cruds
...
class TicketService:
    ...
    async def create_ticket(
        ...
        # Grant initial access to all bosses
        bosses: List[User] = await qb.blank(model=User).base().filter(User.role == UserRole.boss).all()
        if bosses:
    ...
    async def delegate_access(
        ...
        await access_qb.add_orm_list([new_access])
        return new_access
