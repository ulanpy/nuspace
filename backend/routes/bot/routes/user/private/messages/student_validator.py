...
async def student_validator(
    ...
    user = m.forward_from
    user_exist: bool = await check_user_by_telegram_id(session=db_session,
                                                       user_id=user.id)
...
async def handle_hidden_forward(m: Message,
                                _: Callable[[str], str]) -> None:
    await m.answer(_("⛔️К сожелению из-за настроек конфиденциальности пользователя это недоступно, но есть другой вариант!"),
