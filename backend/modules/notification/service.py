    ...
    async def notify_new_ticket_to_bosses(self, ticket: Ticket, bosses: List[User]):
        ...
        notifications_data = [
            RequestNotiification(
                ...
                url=f"{self.infra.config.PUBLIC_WEBHOOK_URL}/sgotinish/sg/ticket/?id={ticket.id}", # of the form https://nuspace.kz/sgotinish/sg/ticket/?id=17
            )
            for boss in bosses
            if boss.telegram_id
        ]
    ...
    async def notify_ticket_updated(self, ticket: Ticket):
        ...
        notifications_data = [
            RequestNotiification(
                ...
                url=f"{self.infra.config.PUBLIC_WEBHOOK_URL}/sgotinish/student/ticket/?id={ticket.id}", # of the form https://nuspace.kz/sgotinish/student/ticket/?id=18
            )
        ]
        if notifications_data:
            await send(
    ...
    async def notify_new_message(self, message: Message):
        ...
        notifications_data = [
            RequestNotiification(
                ...
                url=f"{self.infra.config.PUBLIC_WEBHOOK_URL}/sgotinish/sg/ticket/?id={ticket.id}" if message.is_from_sg_member else f"{self.infra.config.PUBLIC_WEBHOOK_URL}/sgotinish/student/ticket/?id={ticket.id}", # of the form https://nuspace.kz/sgotinish/sg/ticket/?id=17 or https://nuspace.kz/sgotinish/student/ticket/?id=18
            )
        ]
        if notifications_data:
            await send(
